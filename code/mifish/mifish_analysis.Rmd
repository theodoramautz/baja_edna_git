...
# Plot
# Latitude Group
p1 <- plot_ordination(ps.hell.pruned.v2, ord.nmds.bray, color = "lat_group") +
  scale_color_manual(
    name   = "Latitude Group",
    values = c(
      "24-25.6"   = "blue",   # South
      "25.7-27.9" = "black",   # Middle
      "28-30"     = "red"     # North
    ),
    breaks = c("24-25.6", "25.7-27.9", "28-30"),
...
# Max Depth
p2 <- plot_ordination(ps.hell.pruned.v2, ord.nmds.bray, color = "MaxDepth") +
...
# Diver
p3 <- plot_ordination(ps.hell.pruned.v2, ord.nmds.bray, color="Diver") +
  theme(
    plot.title = element_text(hjust = 0.5)
    ) +
  labs(
...
# Bottom Time
p4 <- plot_ordination(ps.hell.pruned.v2, ord.nmds.bray, color = "BottomExposureTime_minutes") +
  scale_color_viridis_c(
    option = "C", 
    direction = -1, 
    name = "Bottom Time (mins)"
    ) +
  theme(
    plot.title = element_text(hjust = 0.5)
    )
...
p_list <- lapply(p_list, function(p) {
  p + theme(
    ...
    legend.key.width = unit(fixed_legend_width, "cm"),
    plot.margin = unit(c(1, 1, 1, 1), "lines")
  )
})

combined_beta_nmds <- plot_grid(plotlist = p_list, ncol = 2, align = "hv", axis = "tblr")
combined_beta_nmds
```
...
# Recode lat group numbers to names
nmds_df <- nmds_df %>%
  mutate(
    lat_group = recode(
      lat_group,
...
combined_nmds <- plot_grid(plotlist = p_list, ncol = 2, align = "hv", axis = "tblr")
```
...
```{r Maps with more regional inserts}
...
# Use unique sites for plotting + labeling (avoids duplicate points/labels)
baja_sites <- baja_metadata %>%
  distinct(Site_ID, lat, lon)
...
# Helper function to build a labeled inset map
make_map <- function(df, xlim, ylim, title = NULL,
                     point_size = 3,
                     label_size = 4,
                     repel_force = 1.2) {

  ggplot() +
    geom_sf(data = world, fill = "gray", color = "black", linewidth = 0.25) +
    coord_sf(xlim = xlim, ylim = ylim, expand = FALSE) +
    geom_point(
      data = df,
      aes(x = lon, y = lat),
      shape = 21, fill = "seagreen3", color = "black",
    ...
    scale_x_continuous(breaks = seq(xlim[1], xlim[2], by = 0.2)) +
...
print(main_map)
# --------------------------------
# 2) Southern lat_group (24–25.6)
# --------------------------------
southern_map_a <- make_map(
  df = baja_sites,
  xlim = c(-110.5, -109.9),
  ylim = c(24.3, 24.7),
  point_size = 3.2,
...
print(southern_map_a)

southern_map_b <- make_map(
  df = baja_sites,
  xlim = c(-110.8, -110.4),
  ylim = c(24.8, 25.2),
  point_size = 3.2,
...
print(southern_map_b)
# --------------------------------
# 3) Middle lat_group (25.7–27.9)
# --------------------------------
middle_map_a <- make_map(
  df = baja_sites,
  xlim = c(-111.6, -111.0),
  ylim = c(25.7, 26.2),
  point_size = 3.2,
...
print(middle_map_b)
# -----------------------------
# 4) Northern lat_group (28–30)
# -----------------------------
northern_map_a <- make_map(
  df = baja_sites,
  xlim = c(-113.0, -112.2),
  ylim = c(28.3, 29.0),
  point_size = 3.2,
...
print(northern_map_a)

northern_map_b <- make_map(
  df = baja_sites,
  xlim = c(-113.8, -113.0),
  ylim = c(28.9, 29.6),
  point_size = 3.2,
...
heat <- ggplot() +
  geom_sf(data = world, fill = "gray", color = "black") + 
  coord_sf(xlim = c(-115, -107), ylim = c(24.2, 29.8), expand = FALSE) +
...
south_heat_a <- ggplot() +
  geom_sf(data = world, fill = "gray", color = "black") + 
  coord_sf(xlim = c(-110.5, -109.9), ylim = c(24.3, 24.7), expand = FALSE) +
  scale_x_continuous(breaks = seq(-110.5, -109.9, by = 0.2)) +
  geom_point(data = similarity_data, aes(x = lon, y = lat, fill = similarity), 
             shape = 21, color = "black", size = 3, stroke = 0.8) + 
  scale_fill_distiller(name = "Similarity", palette = "PuRd", direction = 2) +
  annotation_scale(location = "bl", width_hint = 0.25, text_cex = 0.9) +
...
south_heat_b <- ggplot() +
  geom_sf(data = world, fill = "gray", color = "black") + 
  coord_sf(xlim = c(-110.8, -110.4), ylim = c(24.8, 25.2), expand = FALSE) +
  scale_x_continuous(breaks = seq(-110.8, -110.4, by = 0.2)) +
  geom_point(data = similarity_data, aes(x = lon, y = lat, fill = similarity), 
             shape = 21, color = "black", size = 3, stroke = 0.8) + 
  scale_fill_distiller(name = "Similarity", palette = "PuRd", direction = 2) +
  annotation_scale(location = "bl", width_hint = 0.25, text_cex = 0.9) +
...
middle_heat <- ggplot() +
  geom_sf(data = world, fill = "gray", color = "black") +
  coord_sf(xlim = c(-112, -110.5), ylim = c(25.7, 26.7), expand = FALSE) +
  scale_x_continuous(breaks = seq(-112, -110.5, by = 0.2)) +
  geom_point(data = similarity_data, aes(x = lon, y = lat, fill = similarity), 
             shape = 21, color = "black", size = 3, stroke = 0.8) +
  scale_fill_distiller(name = "Similarity", palette = "PuRd", direction = 2) +
  annotation_scale(location = "bl", width_hint = 0.25, text_cex = 0.9) +
...
middle_heat_a <- ggplot() +
  geom_sf(data = world, fill = "gray", color = "black") +
  coord_sf(xlim = c(-111.6, -111.0), ylim = c(25.7, 26.2), expand = FALSE) +
  scale_x_continuous(breaks = seq(-111.6, -111.0, by = 0.2)) +
  geom_point(data = similarity_data, aes(x = lon, y = lat, fill = similarity), 
             shape = 21, color = "black", size = 3, stroke = 0.8) +
  scale_fill_distiller(name = "Similarity", palette = "PuRd", direction = 2) +
  annotation_scale(location = "bl", width_hint = 0.25, text_cex = 0.9) +
...
middle_heat_b <- ggplot() +
  geom_sf(data = world, fill = "gray", color = "black") +
  coord_sf(xlim = c(-111.8, -111.2), ylim = c(26.4, 26.9), expand = FALSE) +
  scale_x_continuous(breaks = seq(-111.8, -111.2, by = 0.2)) +
  geom_point(data = similarity_data, aes(x = lon, y = lat, fill = similarity), 
             shape = 21, color = "black", size = 3, stroke = 0.8) +
  scale_fill_distiller(name = "Similarity", palette = "PuRd", direction = 2) +
  annotation_scale(location = "bl", width_hint = 0.25, text_cex = 0.9) +
...
# Northern lat_group (28-30)
northern_heat <- ggplot() +
  geom_sf(data = world, fill = "gray", color = "black") +
  coord_sf(xlim = c(-114, -112), ylim = c(28.3, 29.6), expand = FALSE) +
  scale_x_continuous(breaks = seq(-114, -112, by = 0.2)) +
  geom_point(data = similarity_data, aes(x = lon, y = lat, fill = similarity), 
             shape = 21, color = "black", size = 3, stroke = 0.8) +
  scale_fill_distiller(name = "Similarity", palette = "PuRd", direction = 2) +
  annotation_scale(location = "bl", width_hint = 0.25, text_cex = 0.9) +
...
northern_heat_a <- ggplot() +
  geom_sf(data = world, fill = "gray", color = "black") +
  coord_sf(xlim = c(-113.0, -112.2), ylim = c(28.3, 29.0), expand = FALSE) +
  scale_x_continuous(breaks = seq(-113.0, -112.2, by = 0.2)) +
  geom_point(data = similarity_data, aes(x = lon, y = lat, fill = similarity), 
             shape = 21, color = "black", size = 3, stroke = 0.8) +
  scale_fill_distiller(name = "Similarity", palette = "PuRd", direction = 2) +
  annotation_scale(location = "bl", width_hint = 0.25, text_cex = 0.9) +
...
northern_heat_b <- ggplot() +
  geom_sf(data = world, fill = "gray", color = "black") +
  coord_sf(xlim = c(-113.8, -113.0), ylim = c(28.9, 29.6), expand = FALSE) +
  scale_x_continuous(breaks = seq(-113.8, -113.0, by = 0.2)) +
  geom_point(data = similarity_data, aes(x = lon, y = lat, fill = similarity), 
             shape = 21, color = "black", size = 3, stroke = 0.8) +
  scale_fill_distiller(name = "Similarity", palette = "PuRd", direction = 2) +
  annotation_scale(location = "bl", width_hint = 0.25, text_cex = 0.9) +
