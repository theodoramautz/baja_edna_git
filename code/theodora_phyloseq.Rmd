...
ggplot() +
  ...
  scale_color_gradient(name = "Similarity", limits = c(0, 1), low = "blue", high = "red") +
...
unique_species_pruned_eDNA <- as.data.frame(unique(pruned_eDNA$Species))
colnames(unique_species_pruned_eDNA) <- "Species"

# Bring back REEF species & common species
REEF_species <- as.data.frame(REEF_species)
colnames(REEF_species) <- "Species"
common_species <- as.data.frame(intersect(unique_species_pruned_eDNA, REEF_species))

# Make Species Venn
species_list <- list(
  PEDS = unique_species_pruned_eDNA$Species,
  RVS = as.character(REEF_species$Species)
)
...
# Make Families Venn
families_list <- list(
  PEDS = unique(pruned_eDNA$Family),
