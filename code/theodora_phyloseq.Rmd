...
alpha_transp <- plot_richness(ps.prop.pruned.v2, 
              ...
              measures=c("Simpson", "Shannon"),  
              color="TransportTime") +
       labs(x = "Transport Time",         
       color = "Transport Time")
alpha_transp
...
p5 <- plot_ordination(ps.hell.pruned.v2, 
                      ord.nmds.bray, 
                      color="BottomExposureTime_minutes", 
                      title="PEDS NMDS Bottom Time") +
  #theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5)) +
  labs(color = "Bottom Time")
p5
...
p6 <- plot_ordination(ps.hell.pruned.v2, 
                      ord.nmds.bray, 
                      color="TransportTime", 
                      title="PEDS NMDS Transport Time") +
  #theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5)) +
  labs(color = "Transport Time")
p6
...
permanova.v3 <- adonis2(dist ~ lat_group + Habitat + MaxDepth + Diver + Time + Transp,
                     ps.perm.data,
                     permutations=999)
permanova.v3

# Full model interactions
permanova.v4 <- adonis2(dist ~ lat_group * Habitat * MaxDepth * Diver * Time * Transp,
                     ps.perm.data,
                     permutations=999)
permanova.v4
```
...
# Full model main effects
permanova.v3 <- adonis2(dist ~ lat_group + Habitat + Depth + Diver,
                     data = as.data.frame(merged_reef_survey1),
                     permutations=999)
permanova.v3

# Full model interaction effects
permanova.v4<- adonis2(dist ~ lat_group * Habitat * Depth * Diver,
                     data = as.data.frame(merged_reef_survey1),
                     permutations=999)
permanova.v4
```
